    ...
    class Graphics
    {
        ...
        public static void InitGraphics()
        {
            ...
            FEZ_Components.FEZTouch.LCDConfiguration lcdConfig = new FEZ_Components.FEZTouch.LCDConfiguration(
                ...
                FEZ_Components.FEZTouch.Orientation.Portrait
                );
        ...
        public class Label : Control
        {
            ...
            {
                fezTouch.FillRectangle(new FEZ_Components.FEZTouch.Rectangle(X_Pos, Y_Pos, Width, Height), BGColor);
                fezTouch.DrawString(X_Pos, Y_Pos, Text, TextColor, BGColor);
            }
        }
        ...
        public class Slider : Control
        {
            ...
            protected override void Draw()
            {
                if (Orientation == eOrientation.Horizontal)
                {
                    fezTouch.FillRectangle(new FEZ_Components.FEZTouch.Rectangle(prev_pos - (HandleSize / 2), Y_Pos, HandleSize, HandleSize), BGColor); // Clear old handle position
                    fezTouch.FillRectangle(new FEZ_Components.FEZTouch.Rectangle(X_Pos + (HandleSize / 2), Y_Pos + (HandleSize / 2), Size - HandleSize, 1), LineColor); // Draw line
                    fezTouch.FillRectangle(new FEZ_Components.FEZTouch.Rectangle(current_pos - (HandleSize / 2), Y_Pos, HandleSize, HandleSize), HandleColor);  // Draw new handle position
                    prev_pos = current_pos;         // Save for clearing, next time we draw
                }
                else
                {
                    fezTouch.FillRectangle(new FEZ_Components.FEZTouch.Rectangle(X_Pos, prev_pos - (HandleSize / 2), HandleSize, HandleSize), BGColor);
                    fezTouch.FillRectangle(new FEZ_Components.FEZTouch.Rectangle(X_Pos + (HandleSize / 2), Y_Pos + (HandleSize / 2), 1, Size - HandleSize), LineColor);
                    fezTouch.FillRectangle(new FEZ_Components.FEZTouch.Rectangle(X_Pos, current_pos - (HandleSize / 2), HandleSize, HandleSize), HandleColor);
                    prev_pos = current_pos;         // Save for clearing, next time we draw
                }
            }

            void fezTouch_TouchMoveEvent(ref FEZ_Components.FEZTouch.Point point)
            {
                int pos_delta;      // The number of pixels we are from the pixel position that represents MinValue

                if (IsPointInControl(point.X, point.Y))
                {
                    if (Orientation == eOrientation.Horizontal)
                    {
                        // Handle edge cases
                        if (point.X < (X_Pos + (HandleSize / 2)))
                            current_pos = X_Pos + (HandleSize / 2);
                        else if (point.X > ((X_Pos + Size) - (HandleSize / 2)))
                    ...
                    {
                        // Handle edge cases
                        if (point.Y < (Y_Pos + (HandleSize / 2)))
                            current_pos = Y_Pos + (HandleSize / 2);
                        else if (point.Y > ((Y_Pos + Size) - (HandleSize / 2)))
        ...
        public class LED : Control
        {
            ...
            protected override void Draw()
            {
                ...
                if (is_on)
                    fezTouch.FillRectangle(new FEZ_Components.FEZTouch.Rectangle(X_Pos, Y_Pos, (Radius * 2), (Radius * 2)), OnColor);
                else
                    fezTouch.FillRectangle(new FEZ_Components.FEZTouch.Rectangle(X_Pos, Y_Pos, (Radius * 2), (Radius * 2)), OffColor);
            }
        ...
        public class TextArea : Control
        {
            ...
            void Init(int x, int y, int w, int h, FEZ_Components.FEZTouch ft, FEZ_Components.FEZTouch.Color bg, FEZ_Components.FEZTouch.Color fg)
            {
                ...
                X_Pos = x;
                Y_Pos = y;
                if ((w <= FEZ_Components.FEZTouch.FONT_WIDTH) || (h <= FEZ_Components.FEZTouch.FONT_HEIGHT))
                ...
                num_chars_per_line = (Width / FEZ_Components.FEZTouch.FONT_WIDTH) - 1;  // Subtract 1 to allow for "margins"
                num_lines_in_window = (Height / FEZ_Components.FEZTouch.FONT_HEIGHT) - 1;
                ...
                lock (lock_obj)
                {
                    ...
                    if (text_lines != null)
                    {
                        for (i = top_text_line_idx; i < text_lines.Length; i++)
                        {
                            ...
                            {
                                fezTouch.DrawString(X_Pos, Y_Pos + ((i - top_text_line_idx) * FEZ_Components.FEZTouch.FONT_HEIGHT), text_lines[i], TextColor, BGColor);
                                curr_line_num++;
                            }
                        }
                    }
                    for (i = 0; i < top_text_line_idx; i++)
                    {
                        if ((text_lines[i] != null) && (text_lines[i] != String.Empty))
                        {
                            fezTouch.DrawString(X_Pos, Y_Pos + ((i + curr_line_num) * FEZ_Components.FEZTouch.FONT_HEIGHT), text_lines[i], TextColor, BGColor);
                        }
                    }
                }
            }
        ...
        public class StateButton : Control
        {
            ...
            public String State1Text
            {
                ...
                set
                {
                    int text_width = value.Length * FEZ_Components.FEZTouch.FONT_WIDTH;

                    if (text_width > Width)
                        throw new ArgumentException();

                    state1_text = value;
                    fezTouch.DrawString(X_Pos + (Width - text_width) / 2, Y_Pos + (Height - FEZ_Components.FEZTouch.FONT_HEIGHT) / 2, value, Text1Color, State1Color);
                }
            }
            ...
            public String State2Text
            {
                ...
                set
                {
                    ...
                    state2_text = value;
                    fezTouch.DrawString(X_Pos + (Width - text_width) / 2, Y_Pos + (Height - FEZ_Components.FEZTouch.FONT_HEIGHT) / 2, value, Text2Color, State2Color);
                }
            }
                      ...
                      String text1, String text2)
            {
                ...
                Text1Color = t1_c;
                Text2Color = t2_c;
                fezTouch.FillRectangle(new FEZ_Components.FEZTouch.Rectangle(X_Pos, Y_Pos, Width, Height), State1Color);
                state1_text = text1;
                state2_text = text2;
                fezTouch.DrawString(X_Pos + (Width - (state1_text.Length * FEZ_Components.FEZTouch.FONT_WIDTH)) / 2, Y_Pos + (Height - FEZ_Components.FEZTouch.FONT_HEIGHT) / 2, state1_text, Text1Color, State1Color);

                fezTouch.TouchDownEvent += new FEZ_Components.FEZTouch.TouchEventHandler(fezTouch_TouchDownEvent);
            }

            void fezTouch_TouchDownEvent(ref FEZ_Components.FEZTouch.Point point)
            {
                FEZ_Components.FEZTouch.Color new_btn_color;
                FEZ_Components.FEZTouch.Color new_txt_color;
                String new_text;

                if (IsPointInControl(point.X, point.Y) == true)
                {
                    ...
                    fezTouch.FillRectangle(new FEZ_Components.FEZTouch.Rectangle(X_Pos, Y_Pos, Width, Height), new_btn_color);
                    fezTouch.DrawString(X_Pos + (Width - (new_text.Length * FEZ_Components.FEZTouch.FONT_WIDTH)) / 2, Y_Pos + (Height - FEZ_Components.FEZTouch.FONT_HEIGHT) / 2, new_text, new_txt_color, new_btn_color);
        ...
        public class MomentaryButton : Control
        {
            ...
            public String Text
            {
                ...
                set
                {
                    ...
                    button_text = value;
                    fezTouch.DrawString(X_Pos + (Width - text_width) / 2, Y_Pos + (Height - FEZ_Components.FEZTouch.FONT_HEIGHT) / 2, value, TextColor, NotPressedColor);
                }
            }
            ...
            void Init(int x, int y, FEZ_Components.FEZTouch ft, int w, int h, FEZ_Components.FEZTouch.Color pc, FEZ_Components.FEZTouch.Color rc, String txt)
            {
                ...
                NotPressedColor = rc;
                TextColor = FEZ_Components.FEZTouch.Color.White;
                fezTouch.FillRectangle(new FEZ_Components.FEZTouch.Rectangle(X_Pos, Y_Pos, Width, Height), NotPressedColor);
                Text = txt;
                fezTouch.DrawString(X_Pos + (Width - (Text.Length * FEZ_Components.FEZTouch.FONT_WIDTH)) / 2, Y_Pos + (Height - FEZ_Components.FEZTouch.FONT_HEIGHT) / 2, Text, TextColor, NotPressedColor);

                fezTouch.TouchDownEvent += new FEZ_Components.FEZTouch.TouchEventHandler(fezTouch_TouchDownEvent);
                fezTouch.TouchUpEvent += new FEZ_Components.FEZTouch.TouchEventHandler(fezTouch_TouchUpEvent);
            }

            void fezTouch_TouchUpEvent(ref FEZ_Components.FEZTouch.Point point)
            {
                if (IsPointInControl(point.X, point.Y) == true)
                {
                    fezTouch.FillRectangle(new FEZ_Components.FEZTouch.Rectangle(X_Pos, Y_Pos, Width, Height), NotPressedColor);
            ...
            void fezTouch_TouchDownEvent(ref FEZ_Components.FEZTouch.Point point)
            {
                ...
                if (IsPointInControl(point.X, point.Y) == true)
                {
                    fezTouch.FillRectangle(new FEZ_Components.FEZTouch.Rectangle(X_Pos, Y_Pos, Width, Height), PressedColor);
                    fezTouch.DrawString(X_Pos + (Width - (Text.Length * FEZ_Components.FEZTouch.FONT_WIDTH)) / 2, Y_Pos + (Height - FEZ_Components.FEZTouch.FONT_HEIGHT) / 2, Text, TextColor, PressedColor);
                    DoButtonPressed();
                }
            }
        }
        ...
        public class ScrollingGraph : Control
        {
            ...
            protected override void Draw()
            {
                ...
                for (i = 0; i < num_bars; i++)
                {
                    ...
                    fezTouch.FillRectangle(new FEZ_Components.FEZTouch.Rectangle(X_Pos + (i * bar_width), (Y_Pos + (Height - val)), bar_width, val), BarColor);
                }
            }
        }
        ...
        public class CheckBox : Control
        {
            ...
            void Init(int x, int y, FEZ_Components.FEZTouch ft, String txt, FEZ_Components.FEZTouch.Color tc, FEZ_Components.FEZTouch.Color bgc, FEZ_Components.FEZTouch.Color cc)
            {
                ...
                fezTouch = ft;
                Text = txt;
                Width = box_size_px + 3 + (txt.Length * FEZ_Components.FEZTouch.FONT_WIDTH);
                Height = box_size_px;
                BGColor = bgc;
                TextColor = tc;
                CheckColor = cc;
                ...
                // Draw text
                fezTouch.DrawString(X_Pos + box_size_px + 3, Y_Pos + (Height / 2) - (FEZ_Components.FEZTouch.FONT_HEIGHT / 2), Text, TextColor, BGColor);

                // Draw frame
                fezTouch.FillRectangle(new FEZ_Components.FEZTouch.Rectangle(X_Pos, Y_Pos, box_size_px, box_size_px), TextColor);
                fezTouch.FillRectangle(new FEZ_Components.FEZTouch.Rectangle(X_Pos + 1, Y_Pos + 1, box_size_px - 2, box_size_px - 2), BGColor);
            ...
            protected override void Draw()
            {
                fezTouch.FillRectangle(new FEZ_Components.FEZTouch.Rectangle(X_Pos + 2, Y_Pos + 2, box_size_px - 4, box_size_px - 4), IsChecked ? CheckColor : BGColor);
            }
        }
        ...
        public class RadioButton : Control
        {
            ...
            public RadioButton(int x, int y, FEZ_Components.FEZTouch ft, String[] lbls, FEZ_Components.FEZTouch.Color tc, FEZ_Components.FEZTouch.Color bgc, FEZ_Components.FEZTouch.Color bc)
            {
                ...
                foreach (String s in lbls)
                {
                    if ((s.Length * FEZ_Components.FEZTouch.FONT_WIDTH) > max_len)
                        max_len = (s.Length * FEZ_Components.FEZTouch.FONT_WIDTH);
                }
                max_len += (box_width_px + 3);
            ...
            void Init(int x, int y, int w, int h, FEZ_Components.FEZTouch ft, String[] lbls, FEZ_Components.FEZTouch.Color tc, FEZ_Components.FEZTouch.Color bgc, FEZ_Components.FEZTouch.Color bc)
            {
                ...
                {
                    y_offset = (i * box_width_px) + (box_width_px / 2) - (FEZ_Components.FEZTouch.FONT_HEIGHT / 2) + i;
                    fezTouch.DrawString(X_Pos + box_width_px + 3, Y_Pos + y_offset, button_labels[i], TextColor, BGColor);
                }

                // Draw Frames
                for (i = 0; i < button_labels.Length; i++)
                {
                    fezTouch.FillRectangle(new FEZ_Components.FEZTouch.Rectangle(X_Pos, Y_Pos + (box_width_px * i) + 1 + i, box_width_px, box_width_px), TextColor);
                    fezTouch.FillRectangle(new FEZ_Components.FEZTouch.Rectangle(X_Pos + 1, (Y_Pos + (box_width_px * i)) + 2 + i, box_width_px - 2, box_width_px - 2), BGColor);
                }
            ...
            void fezTouch_TouchDownEvent(ref FEZ_Components.FEZTouch.Point point)
            {
                ...
                if (IsPointInControl(point.X, point.Y))
                {
                    idx = (point.Y - Y_Pos) / (box_width_px + 2);
                    if (idx != selected_idx)
                    {
                        selected_idx = idx;
                        Draw();
            ...
            protected override void Draw()
            {
                ...
                for (i = 0; i < button_labels.Length; i++)
                {
                    if (i == selected_idx)
                    {
                        fezTouch.FillRectangle(new FEZ_Components.FEZTouch.Rectangle(X_Pos + 2, (Y_Pos + (box_width_px * i)) + 3 + i, box_width_px - 4, box_width_px - 4), ButtonColor);
                    }
                    else
                    {
                        fezTouch.FillRectangle(new FEZ_Components.FEZTouch.Rectangle(X_Pos + 2, (Y_Pos + (box_width_px * i)) + 3 + i, box_width_px - 4, box_width_px - 4), BGColor);
                    }
                }
            }
        }
